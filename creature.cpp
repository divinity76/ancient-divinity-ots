...
void Creature::death(std::string killer, std::string altKiller)
{
	if (Player* player = this->getPlayer())
		IOPlayer::instance()->saveDeath(player->getName(), std::time(NULL), player->getLevel(), killer, altKiller);
}
	...
	if (g_config.getNumber(ConfigManager::STORE_DEATHS) != 0 ) {
		if (getPlayer()) {
			if (lastHitCreature && mostDamageCreature) {
				death(lastHitCreature->getName(), mostDamageCreature->getName());
			}
		}
	}
...
uint64_t Creature::getGainedExperience(Creature* attacker, bool useMultiplier /*= true*/) const
{
	...
	if(Player* player = attacker->getPlayer()){
		if(useMultiplier){
			return (uint64_t)std::floor(getDamageRatio(attacker) * lostExperience * 
				g_config.getNumber(ConfigManager::RATE_EXPERIENCE) * player->exp_multiplier);
		}
	}

#ifdef __XID_EXPERIENCE_STAGES__
	if (Player* player = attacker->getPlayer()) {
		return (uint64_t)std::floor(getDamageRatio(attacker) * lostExperience * g_game.getExperienceStage(player->getLevel()));
	}
	else {
